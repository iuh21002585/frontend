...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.code === 'ECONNABORTED') {
      ...
      Object.assign(error, {
        isTimeout: true,
        timeoutMessage: 'Kết nối đến máy chủ đã quá thời gian chờ. Vui lòng thử lại sau.'
      });
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        if (error.response.status === 502) {
          ...
          Object.assign(error, {
            isBackendDown: true,
            backendErrorMessage: 'Máy chủ tạm thời không khả dụng (502).'
          });
          ...
          Object.assign(error, {
            isTimeout: true,
            timeoutMessage: 'Máy chủ phản hồi quá chậm (504).'
          });
          ...
          Object.assign(error, {
            isTimeout: true,
            timeoutMessage: 'Yêu cầu đã hết thời gian chờ (408).'
          });
        }
        ...
        Object.assign(error, {
            isBackendDown: true,
            backendErrorMessage: 'Không nhận được phản hồi từ máy chủ.'
        });
      }
    }
